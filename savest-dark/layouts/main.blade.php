                ...
                <div class="col-auto">
                    @if (! empty(settings('company.logo.' . settings('logo_positions.header'))))
                        <div class="header-logo">
                            <a href="{{ route('home') }}">
                                <img class="logo-main" src="{{ settings('company.logo.' . settings('logo_positions.header')) }}" height="58" alt="{{ settings('company.name') }}" />
                            </a>
                        </div>
                    @endif
                </div>
...
<div class="header-bottom sticky-header">
    <div class="container-fluid header-container-fluid py-2">
        <div class="row justify-content-between align-items-center">
            @if (($template_settings['header']['logo_position']['value'] ?? '') == 'inline_menu' && ! empty($template_settings['header']['logo_position']['apply']) && ($template_settings['header']['menu_position']['value'] ?? '') != 'burger')
                <div class="col-auto">
                    @if (! empty(settings('company.logo.' . settings('logo_positions.header'))))
                        <div class="header-logo">
                            <a href="{{ route('home') }}">
            ...
            @if (($template_settings['header']['logo_position']['value'] ?? '') != 'inline_menu' || ($template_settings['header']['menu_position']['value'] ?? '') == 'burger')
            ...
            <div class="col-auto d-flex align-items-center gap-6 gap-lg-0">
                <button class="btn-menu d-flex ml-4 d-lg-none" type="button" data-bs-toggle="offcanvas" data-bs-target="#AsideOffcanvasMenu" aria-controls="AsideOffcanvasMenu">
                    <span></span>
                    <span></span>
                    <span></span>
                </button>
                ...
                <div class="footer-widget text-center text-md-start">
                    @if (! empty(settings('company.logo.' . settings('logo_positions.footer'))))
                        <a href="{{ route('home') }}" class="footer-widget-logo me-auto me-md-0 ms-auto ms-md-0">
                            <img class="logo-main" src="{{ settings('company.logo.' . settings('logo_positions.footer')) }}" height="58" alt="{{ settings('company.name') }}" />
                        </a>
                    @endif
